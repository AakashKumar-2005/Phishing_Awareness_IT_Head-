...
def track_click():
    ...
    if email:
        update_csv(email, name, 'Seen the email and Opened it')
...
def update_csv(email, name, status):
    """
    Safely update the CSV file with the email status while retaining the Name.
    """
            ...
            with open(CSV_FILE, mode='r') as file:
                ...
                for row in reader:
                    if row['Email'] == email:
                        ...
                        # Only update the name if it's provided and not empty
                        if name:
                            row['Name'] = name
                        updated = True
                    rows.append(row)
